...
def push(name,tree,mass):
    ...
    for key in tree.keys():
        ...
        for i in node.Dep:
            dep += int((dep != ""))*',' + str(i)
        f.write('\t<task index='+ocr(key)+' maj_fr='+ocr(node.maj)+' prio='+ocr(key)+' bcet='+ocr(node.bcet)+' wcet='+ocr(node.wcet)+' period='+ocr(node.per)+' deadline='+ocr(node.per)+' proc='+ocr(node.proc)+' Dep="'+dep+'" Inf="'+inf+'" />\n')
    for node in mass:
        #print(mass)
