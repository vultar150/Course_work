...
class Task
{
    public:
        ...
        bool _isTarget;
        vector<Task*> _tExcl; // direct pass
        vector<Task*> _tExcl2; // reverse pass
        vector<Task*> _predecessors;
        vector<Task*> _successors;
        vector<Task*> _tPmtor;
        vector<Task*> _hp;
        map<int, int> _delays;
...
class TaskGraph: public map<int,Task*>
{
    public:
        int _id;
        int _period;
...
class Processors: public map<int, vector<TaskPosition> >
{
    public:
        void outInfo() const;
};
